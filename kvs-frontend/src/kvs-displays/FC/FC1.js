    ...
    <div className="App kvs-background">
      <div className={`orders-container columns-${columns}`}>
        ...
        <div className='station-statistics'>
          <span className='station-stats'>All / FC Side 1 <span className={`station-status ${currentStation.status}`}>{currentStation.status}</span> {averageTimestampDifferenceLastHour(servedOrders, stationName)}/{averageTimestampDifferenceLast24Hours(servedOrders, stationName)}</span>
        </div>
