...
function Admin() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_SERVER_ADDRESS}:5000/categories`);
  ...
  const fetchCurrentBusinessDay = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_SERVER_ADDRESS}:5000/store/latest`);
      if (!response.data) {
  ...
  const handleAddOrder = async () => {
    ...
    /*
    try {
      await axios.post('http://${process.env.REACT_APP_SERVER_ADDRESS}:5000/orders', orderToAdd)
  ...
  const handleAddItem = async (menuItem) => {
    try {
      if (menuItem) {
        await axios.post(`${process.env.REACT_APP_SERVER_ADDRESS}:5000/options`, {
          name: menuItem.name,
  ...
  const handleAddCategory = async (categoryName) => {
    try {
      if (categoryName) {
        await axios.post(`${process.env.REACT_APP_SERVER_ADDRESS}:5000/categories`, {
  ...
  const importStations = async () => {
    for (let kvsDisplay of kvsDisplays) {
      ...
      if (!stationExists) {
        await axios.post(`${process.env.REACT_APP_SERVER_ADDRESS}:5000/stations`, {
  ...
  const openNewBusinessDay = async () => {
    ...
    await axios.post(`${process.env.REACT_APP_SERVER_ADDRESS}:5000/store`, {
